...
class OneHotMLP:
    ...
    def __init__(self, n_features, h_layers, out_size, savedir, labels_text,
            branchlist, diff_param, act_func='tanh'):
...
#        self.bg_weight = bg_weight
    ...
    def train(self, train_data, val_data, optimizer='Adam', epochs = 10, batch_size = 100,
            ...
            batch_decay_options=[], ttH_penalty=1e-5, gpu_usage=None):
        ...
        with train_graph.as_default():
            ...
            w_2 = 1.0 / batch_size * (self.ttH_penalty * tf.add(self.diff_param * 
                tf.multiply(tf.nn.softmax(y_)[:,0], (tf.subtract(1.0, y[:,0]))), 
                tf.multiply( tf.subtract(1.0, tf.nn.softmax(y_)[:,0]), y[:,0])))
            ...
            #         l2_reg, name='loss')
            loss = tf.add(tf.reduce_sum(tf.add(tf.multiply(w, xentropy),w_2)), 
                    l2_reg, name='loss')
            # loss = tf.reduce_mean(w_2)
            # optimizer
